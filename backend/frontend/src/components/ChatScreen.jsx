  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      activeSocket.on('message-receive-encrypted', async ({ senderId, encryptedPayload }) => {
        ...
        // Need to use the latest user list for this.
        setUserList(currentUsers => {
          const sender = currentUsers.find(u => u.userId === senderId);
          if (sender && sender.publicKey) {
             decryptPayload(encryptedPayload, keys.keyPair.privateKey, sender.publicKey).then(decrypted => {
                if (decrypted) {
